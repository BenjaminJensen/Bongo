...
uint64_t known_device[NUM_KNOWN_DEVICES] = {0xad0417633798ff28};
...
static void temp_task(void* parms) {
	...
	if (num_devices > 0)
	{
		...
		while (1)
		{
			...
			for (int i = 0; i < num_devices; ++i)
			{
				len = sprintf(cPayload, "%.1f", readings[i]);

				// MQTT publish
				rc = aws_client_pub(pubSlot, cPayload, len);
				if(rc != SUCCESS) {
					ESP_LOGW(TAG, "Temp updated publish error: %d", rc);
				} else {
					ESP_LOGI(TAG, "Temp updated publish : %.1f", readings[i]);
				}
			}
			/*
